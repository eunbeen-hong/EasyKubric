...
def fixed_randomness(scene, randomness, key, value, idx=None):
    if idx != None: # List
        value = randomness[key][idx] if randomness else value
        scene.randomness.setdefault(key, []).append(value)
        return value
    else:
        value = randomness[key] if randomness else value
        scene.randomness[key] = value
        return value
...
# Configuration for the source of the asstatic_pqssets
...
# --------------------------------------------------------------------------------------------------
# Added/moved Configuration
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")    # e.g., 20250917_123045
parser.add_argument(
        "--job-dir", 
        type=str, 
        default=f"./outputs/{timestamp}"
)
...
parser.add_argument("--metadata", type=str, default=None, help="path to metadata.json file to remove randomness")
parser.add_argument("--min_amb_light", type=float, default=0.01, help="min ambient light strength")
parser.add_argument("--max_amb_light", type=float, default=1.0, help="max ambient light strength")
# --------------------------------------------------------------------------------------------------
...
randomness = json.load(open(FLAGS.metadata))['randomness'] if FLAGS.metadata else None
...
hdri_id = fixed_randomness(scene, randomness, "background", hdri_id)
...
amb_light = rng.uniform(FLAGS.min_amb_light, FLAGS.max_amb_light)
amb_light = fixed_randomness(scene, randomness, "amb_light", amb_light)
renderer._set_ambient_light_hdri(background_hdri.filename, strength=amb_light)
...
if FLAGS.camera == "fixed_random":
    camera_pos = kb.sample_point_in_half_sphere_shell(
            inner_radius=7., outer_radius=9., offset=0.1)
    
    camera_pos = fixed_randomness(scene, randomness, "camera_pos", camera_pos)
    ...
    camera_start, camera_end = fixed_randomness(scene, randomness, "camera_start_end", (camera_start, camera_end))
    
    if is_panning:
        lookat_start, lookat_end = get_linear_lookat_motion_start_end()
        lookat_start, lookat_end = fixed_randomness(scene, randomness, "lookat_start_end", (lookat_start, lookat_end))
...
num_static_objects = fixed_randomness(scene, randomness, "num_static_objects", num_static_objects)
    
logging.info("Randomly placing %d static objects:", num_static_objects)
for i in range(num_static_objects):
    asset_id = fixed_randomness(scene, randomness, "static_asset_ids", rng.choice(active_split), idx=i)
    scale = fixed_randomness(scene, randomness, "static_scales", rng.uniform(0.75, 3.0), idx=i)
    ...
    pos, qua = fixed_randomness(scene, randomness, "static_pqs", (obj.position, obj.quaternion), idx=i)
    obj.position, obj.quaternion = pos, qua
    obj.friction = 1.0
    obj.restitution = 0.0
...
num_dynamic_objects = fixed_randomness(scene, randomness, "num_dynamic_objects", num_dynamic_objects)

logging.info("Randomly placing %d dynamic objects:", num_dynamic_objects)
for i in range(num_dynamic_objects):
    asset_id = fixed_randomness(scene, randomness, "dynamic_asset_ids", rng.choice(active_split), idx=i)
    scale = fixed_randomness(scene, randomness, "dynamic_scales", rng.uniform(0.75, 3.0), idx=i)
    velocity = fixed_randomness(scene, randomness, "dynamic_velos", \
                            (rng.uniform(*VELOCITY_RANGE) - [obj.position[0], obj.position[1], 0]), idx=i)
    ...
    pos, qua = fixed_randomness(scene, randomness, "dynamic_pqs", (obj.position, obj.quaternion), idx=i)
    obj.position, obj.quaternion = pos, qua
...
kb.write_json(filename=output_dir / "metadata.json", data={
    ...
    "randomness": kb.get_randomness_metadata(scene),
})
