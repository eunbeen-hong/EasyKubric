...
def get_camera_matrices(
  ...
  intrinsics_pixel = []
  matrix_world = []
  assert cam_quaternions.shape[0] == num_frames, f"{cam_quaternions.shape[0]} != {num_frames}"
  for frame_idx in range(cam_quaternions.shape[0]):
    focal_length = tf.cast(cam_focal_length, tf.float32)  # in mm
    sensor_width = tf.cast(cam_sensor_width, tf.float32)  # in mm
    image_width = tf.cast(input_size[0], tf.float32)      # in pixels
    image_height = tf.cast(input_size[1], tf.float32)     # in pixels

    # Original normalized focal lengths
    f_x_norm = focal_length / sensor_width
    f_y_norm = (focal_length / sensor_width) * (image_width / image_height)
    p_x_norm = 0.5
    p_y_norm = 0.5

    # Intrinsic matrix in normalized units
    intrinsics.append(
        tf.stack([
            tf.stack([f_x_norm, 0., -p_x_norm]),
            tf.stack([0., -f_y_norm, -p_y_norm]),
            tf.stack([0., 0., -1.]),
        ])
    )

    # Compute focal lengths in pixel units
    f_x_pixel = (focal_length / sensor_width) * image_width
    f_y_pixel = (focal_length / sensor_width) * image_height
    p_x_pixel = image_width / 2.0
    p_y_pixel = image_height / 2.0

    # Intrinsic matrix in pixel units
    intrinsics_pixel.append(
        tf.stack([
            tf.stack([f_x_pixel, 0., p_x_pixel]),
            tf.stack([0., f_y_pixel, p_y_pixel]),
            tf.stack([0., 0., 1.]),
        ])
    )

    # World transformation matrix remains the same
  ...
  return (
      ...
      tf.cast(tf.stack(intrinsics_pixel), tf.float32),
  )
...
def single_object_reproject(
  ...
  # Now, compute normals in camera space
  if quat is not None:
      # It's an object
      # Compute object rotation matrices
      obj_rot_mats = quat2rot(quat)  # [num_frames, 3, 3]

      query_obj_rot_mat = tf.gather(obj_rot_mats, frame_for_pt)
      obj_frame_normals = tf.einsum(
          'boi,bi->bo',
          tf.linalg.inv(query_obj_rot_mat),
          normals,
      ...
      # It's the background
      # normals are in world coordinates
      # Expand normals to [num_points, num_frames, 3]
      world_frame_normals_frames = tf.tile(
          normals[:, tf.newaxis, :],
          [1, num_frames, 1],
      )  # [num_points, num_frames, 3]

  # Now, transform normals from world to camera coordinates
  homo_transform = tf.linalg.inv(camera['matrix_world'])  # [num_frames, 4, 4]
  cam_rot_mats = homo_transform[:, :3, :3]  # [num_frames, 3, 3]

  # Transform normals to camera coordinates
  normals_cam = tf.einsum('fij,bfj->bfi', cam_rot_mats, world_frame_normals_frames)  # [num_points, num_frames, 3]

  # Normalize the normals
  normals_cam = tf.math.l2_normalize(normals_cam, axis=-1)

  return obj_reproj, tf.logical_or(faces_away, obj_occ), depth_proj, normals_cam, world_pos
...
def get_points_on_a_grid_tf(
    size,
    extent,
    center=None,
):
    """
    Get a grid of points covering a rectangular region in TensorFlow.

    Args:
        size (int): Grid size (number of points along each axis).
        extent (tuple): Height and width of the grid extent as (H, W).
        center (tuple, optional): Grid center as (c_y, c_x). Defaults to center of extent.

    Returns:
        tf.Tensor: Grid of points with shape (1, size*size, 2), where each point is (x, y).
    """
    H, W = extent
    if size == 1:
        # Single point at the center
        center_point = tf.constant([W / 2.0, H / 2.0], dtype=tf.float32)
        return tf.reshape(center_point, [1, 1, 2])

    if center is None:
        center = (H / 2.0, W / 2.0)

    margin = W / 64.0
    y_min = margin - (H / 2.0) + center[0]
    y_max = (H / 2.0) + center[0] - margin
    x_min = margin - (W / 2.0) + center[1]
    x_max = (W / 2.0) + center[1] - margin

    # Generate linearly spaced coordinates
    y_range = tf.linspace(y_min, y_max, size)
    x_range = tf.linspace(x_min, x_max, size)

    # Create a meshgrid
    grid_y, grid_x = tf.meshgrid(y_range, x_range, indexing='ij')

    # Stack and reshape to (1, size*size, 2)
    grid = tf.stack([grid_x, grid_y], axis=-1)  # Shape: (size, size, 2)
    grid = tf.reshape(grid, [1, size * size, 2])
...
def track_points(
    ...
    # print(f"cnt : {cnt.numpy()}")
    # print(f"num_to_sample : {num_to_sample.numpy()}")
    
    num_to_sample.set_shape([max_seg_id])
    # print(f"num_to_sample (reshape) : {num_to_sample.numpy()}")
    intrinsics, matrix_world, intrinsics_pixel = get_camera_matrices(
        cam_focal_length,
        cam_positions,
        cam_quaternions,
        cam_sensor_width,
    ...
    frame_grid = tf.fill(tf.shape(y_flat), 0) # query from frame 0
    pix_coords = tf.stack([frame_grid, y_flat, x_flat], axis=1)
    ...
    for i in range(max_seg_id):
      ...
      # print(mask.shape, segmentations_box.shape)
      # print(object_coordinates_box.shape)
      ...
      if obj_id == -1:
        ...
        quat = None
        frame_for_pt = None
        is_bkgd = tf.ones([tf.shape(pt)[0]], dtype=tf.bool)
        ...
        is_bkgd = tf.zeros([tf.shape(pt)[0]], dtype=tf.bool)
      ...
      # print(f"Object {i} has {tf.shape(pt)[0]} points.")

      # Finally, compute the reprojections for this particular object.
      obj_reproj, obj_occ, reproj_depth, normals_cam, world_coords = tf.cond(
          tf.shape(pt)[0] > 0,
          functools.partial(
      ...
      # print(f"Object {i} reprojection shape: {obj_reproj.shape}, "
      #       f"occlusion shape: {obj_occ.shape}, "
      #       f"reprojection depth shape: {reproj_depth.shape}, "
      #       f"normals shape: {normals_cam.shape}, "
      #       f"world coords shape: {world_coords.shape}")
      
      # print occlusion status
      # print(f"Object {i} occlusion status: {obj_occ.numpy().sum()} occluded points out of {obj_occ.shape} total points.")
      
      all_reproj.append(obj_reproj)
      all_occ.append(obj_occ)
      all_reproj_depth.append(reproj_depth)
      all_normals.append(normals_cam)
      all_world_coords.append(world_coords)
      
      # customized
      is_bkgd_lst.append(is_bkgd)

    # Grid sampling code
    if grid_frames is not None and grid_size is not None:
        # Ensure frames are within bounds and convert to Python list
    
        grid_frames = [int(frame) for frame in grid_frames]
        grid_frames = [max(0, min(frame, num_frames - 1)) for frame in grid_frames]

        for frame in grid_frames:

            # Generate grid points using TensorFlow equivalent
            grid_extent = (tf.cast(window[2] - window[0], tf.float32),
                          tf.cast(window[3] - window[1], tf.float32))
            grid_center = (tf.cast((window[0] + window[2]) / 2.0, tf.float32),
                           tf.cast((window[1] + window[3]) / 2.0, tf.float32))

            grid_points = get_points_on_a_grid_tf(size=grid_size, extent=grid_extent, center=grid_center)
            # grid_points shape: (1, grid_size*grid_size, 2)

            # Repeat grid_points for the current frame
            grid_points = tf.tile(grid_points, [1, 1, 1])  # Shape remains (1, grid_size*grid_size, 2)

            # Extract [x, y] and ensure they are integers
            grid_x = tf.cast(tf.round(grid_points[:, :, 0]), tf.int32)
            grid_y = tf.cast(tf.round(grid_points[:, :, 1]), tf.int32)


            y_grid_flat = tf.reshape(grid_y, [-1])
            x_grid_flat = tf.reshape(grid_x, [-1])

            # Combine x and y with the current frame
            frame_grid = tf.fill(tf.shape(y_grid_flat), frame)
            pix_coords_grid = tf.stack([frame_grid, y_grid_flat, x_grid_flat], axis=1)  # [num_points, 3]

            # Get segmentation at these pixel coordinates
            idx = frame * (input_size[0] * input_size[1]) + y_grid_flat * input_size[1] + x_grid_flat
            seg_flat = tf.reshape(segmentations, [-1])  # [num_frames * height * width]
            seg_grid = tf.gather(seg_flat, idx)

            # Process each object separately
            for i in range(max_seg_id):
                obj_id = i - 1
                mask = tf.equal(seg_grid, i)
                pt_coords = tf.boolean_mask(pix_coords_grid, mask)

                normals_all = tf.reshape(surface_normal_map[frame], [-1, 3])
                normals_indices = y_grid_flat * input_size[1] + x_grid_flat
                normals_indices = tf.boolean_mask(normals_indices, mask)
                normals = tf.gather(normals_all, normals_indices)

                trust_sn_all = tf.reshape(trust_sn[frame], [-1, 1])
                trust_sn_mask = tf.gather(trust_sn_all, normals_indices)

                if obj_id == -1:
                    # For background object, unproject points from depth map
                    pt = unproject(pt_coords[:, 1:], get_camera(frame), depth_map[frame])
                    ...
                    # For other objects, get object coordinates
                    obj_coords_frame = object_coordinates[frame]
                    pt_coords_indices = pt_coords[:, 1] * input_size[1] + pt_coords[:, 2]
                    pt_coords_indices = tf.cast(pt_coords_indices, tf.int32)
                    obj_coords_frame_flat = tf.reshape(obj_coords_frame, [-1, 3])
                    pt = tf.gather(obj_coords_frame_flat, pt_coords_indices)
                    pt = pt / np.iinfo(np.uint16).max - .5
                    frame_for_pt = pt_coords[:, 0]
                ...
                # Get depth at the chosen points
                depth_flat = tf.reshape(depth_map[frame], [-1])
                idx_depth = y_grid_flat * input_size[1] + x_grid_flat
                idx_depth = tf.boolean_mask(idx_depth, mask)
                pt_depth = tf.gather(depth_flat, idx_depth)
                chosen_points.append(pt_coords)
                chosen_points_depth.append(pt_depth)

                # Compute the reprojections for this particular object
                obj_reproj, obj_occ, reproj_depth, normals_cam, _ = single_object_reproject(
                    bbox_3d=bbox,
                    pt=pt,
                    pt_segments=i,
                    camera=get_camera(),
    ...
    # for world space - Koo
    all_world_coords = tf.concat(all_world_coords, axis=0)
    ...
    all_normals_view = tf.concat(all_normals, axis=0)
    is_bkgd = tf.concat(is_bkgd_lst, axis=0)
    ...
    homo_transform = tf.linalg.inv(matrix_world)  # [num_frames, 4, 4]
    cam_rot_mats = homo_transform[:, :3, :3]      # [num_frames, 3, 3]
    cam_trans_mats = homo_transform[:, :3, 3]     # [num_frames, 3]

    # Reshape normals to [num_frames, height * width, 3]
    normals_flat = tf.reshape(surface_normal_map, [num_frames, -1, 3])  # [num_frames, num_pixels, 3]

    # Apply camera rotation matrices to normals
    normals_cam_flat = tf.einsum('fij,fnj->fni', cam_rot_mats, normals_flat)

    # Normalize the normals
    normals_cam_flat = tf.math.l2_normalize(normals_cam_flat, axis=-1)

    # Reshape back to [num_frames, height, width, 3]
    normals_map = tf.reshape(normals_cam_flat, tf.shape(surface_normal_map))  # [num_frames, height, width, 3]
    
    # resize to train size
    normals_map = tf.image.resize(normals_map, (512, 512))
    ...
    return (
        ...
        all_relative_depth,
        all_reproj_depth,
        all_normals_view,  # Return the normals for all points across all frames
        normals_map,
        intrinsics_pixel,
        cam_rot_mats,
        cam_trans_mats,
...
def add_tracks(
    ...
    max_sampled_frac=0.1,
    snap_to_occluder=False,
    # Changed parameter: grid_size instead of grid_stride
    grid_frames=None,
    grid_size=None,
):
    """Track points in 2D using Kubric data.
    ...
    Args:
        ...
        train_size: Tuple of 2 ints. Cropped output will be at this resolution.
          Ignored if random_crop is False.
        vflip: Bool. Whether to vertically flip images and tracks (to test generalization).
        ...
        max_seg_id: Int. The maximum segment id in the video.
        max_sampled_frac: Float. The maximum fraction of points to sample from each
          object, out of all points that lie on the sampling grid.
        snap_to_occluder: Bool. If true, query points within 1 pixel of occlusion
          boundaries will track the occluding surface rather than the background.
          This results in models which are biased to track foreground objects
          instead of background. Whether this is desirable depends on downstream
          applications.
        grid_frames: List of int. Frame indices where grid sampling should occur.
        grid_size: Int. The number of grid points along each axis (height and width).

    Returns:
        A dict with the following keys:
        - 'query_points': A set of queries, randomly sampled from the video (with a bias
          toward objects), of shape [num_points, 3]. Each point is [t, y, x], where
          t is time. Points are in pixel/frame coordinates.
        - 'target_points': The trajectory for each query point, of shape
          [num_points, num_frames, 2]. Each point is [x, y] in pixel coordinates.
        - 'relative_depth': Relative depth of each point across frames,
          shape [num_points, num_frames].
        - 'occluded': Occlusion flag for each point, of shape [num_points, num_frames].
          This is a boolean, where True means the point is occluded.
        - 'video': The cropped video, normalized into the range [-1, 1].

    """
    shp = data['video'].shape.as_list()
    num_frames = shp[0]
    if any([s % sampling_stride != 0 for s in shp[1:-1]]):
    ...
    # Call the updated track_points function with new parameters
    query_points, target_points, \
      occluded, relative_depth, absolute_depth, \
        all_normals, normals_map, intrinsics, cam_rot, cam_trans, \
          all_worlds_coords, is_bkgd = track_points(
        object_coordinates=data['object_coordinates'],
        depth=data['depth'],
        depth_range=data['metadata']['depth_range'],
        segmentations=data['segmentations'],
        surface_normals=data['normal'],
        bboxes_3d=data['instances']['bboxes_3d'],
        obj_quat=data['instances']['quaternions'],
        cam_focal_length=data['camera']['focal_length'],
        cam_positions=data['camera']['positions'],
        cam_quaternions=data['camera']['quaternions'],
        cam_sensor_width=data['camera']['sensor_width'],
        window=crop_window,
        ...
        # Pass the new grid sampling parameters
        grid_frames=grid_frames,
        grid_size=grid_size,
    )
    video = data['video']

    # Adjust shapes accordingly
    shp = video.shape.as_list()
    num_points = tf.shape(query_points)[0]
    query_points.set_shape([None, 3])
    target_points.set_shape([None, num_frames, 2])
    relative_depth.set_shape([None, num_frames])
    absolute_depth.set_shape([None, num_frames])
    occluded.set_shape([None, num_frames])
    all_normals.set_shape([None, num_frames, 3])

    # Crop the video to the sampled window, matching the coordinate frame
    ...
    # Crop and resize normals_map
    normals_map = tf.slice(normals_map, start, size)
    normals_map = tf.image.resize(tf.cast(normals_map, tf.float32), train_size)
    normals_map.set_shape([num_frames, train_size[0], train_size[1], 3])

    # Adjust the intrinsics according to the crop and resize
    offset_y = tf.cast(crop_window[0], tf.float32)
    offset_x = tf.cast(crop_window[1], tf.float32)
    crop_height = tf.cast(crop_window[2] - crop_window[0], tf.float32)
    crop_width = tf.cast(crop_window[3] - crop_window[1], tf.float32)

    scale_y = tf.cast(train_size[0], tf.float32) / crop_height
    scale_x = tf.cast(train_size[1], tf.float32) / crop_width

    # Reshape intrinsics to ensure correct dimensions
    intrinsics = tf.reshape(intrinsics, [-1, 3, 3])  # Shape: [num_frames, 3, 3]
    ...
    # Adjust principal point due to crop
    cx = cx - offset_x
    cy = cy - offset_y

    # Adjust focal lengths and principal points due to resize
    fx = fx * scale_x
    fy = fy * scale_y
    cx = cx * scale_x
    cy = cy * scale_y

    # Reconstruct the intrinsics matrix
    zeros = tf.zeros_like(fx)
    ones = tf.ones_like(fx)

    new_intrinsics = tf.stack([
        tf.stack([fx, zeros, cx], axis=-1),
        tf.stack([zeros, fy, cy], axis=-1),
        tf.stack([zeros, zeros, ones], axis=-1)
    ], axis=-2)  # Shape: [num_frames, 3, 3]

    intrinsics = new_intrinsics  # Update intrinsics with adjusted values
    ...
    res = {
        ...
        'cam_rot' : cam_rot,
        'cam_trans' : cam_trans,
        'world_coords' : all_worlds_coords,
        'is_bkgd' : is_bkgd,
    }
    return res
...
def create_point_tracking_dataset(
  ...
  ds = tfds.load(
      'movi_f/512x512',
      data_dir='/media/hdd2/PointTracking',
