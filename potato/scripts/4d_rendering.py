...
def get_flags():
    ...
    parser.add_argument("--gpu_tile_size", type=int, default=256,
                        help="Tile size for GPU rendering. Increase this value if you have more GPU memory.")
    # --------------------------------------------------------------------------------------------------
...
def simulate(FLAGS):
    ...
    renderer = Blender(scene, scratch_dir, use_denoising=True, samples_per_pixel=64, gpu_tile_size=FLAGS.gpu_tile_size)
    ...
    renderer.set_motion_blur(motion_blur)
    ...
    if FLAGS.camera == "fixed_random":
        ...
        for frame in range(FLAGS.frame_start - 1, FLAGS.frame_end + 2):
            ...
            traj_positions.append(interp * np.array(camera_start) +
    ...
    return {
            'scene': scene,
            'renderer': renderer,
            'camera_traj': (traj_positions, traj_lookats),
...
def render(cam_id, pos_look, scene, renderer, camera_traj, collisions, FLAGS, rng, output_dir):
    ...
    output_dir_i = output_dir / f"camera_{cam_id:03d}"
    # linearly interpolate the camera position between these two points
    # while keeping it focused on the center of the scene
    # we start one frame early and end one frame late to ensure that
    # forward and backward flow are still consistent for the last and first frames
    for frame in range(FLAGS.frame_start - 1, FLAGS.frame_end + 2):
        scene.camera.position = pos_look[0]
        scene.camera.look_at(pos_look[1])
    ...
    # --- Postprocessing
    logging.info("Post-processing the results ...")
    ...
    # Save to image files
    logging.info("Saving the results to '%s' ...", output_dir_i)
    kb.write_image_dict(data_stack, output_dir_i)
    kb.post_processing.compute_bboxes(data_stack["segmentation"], visible_foreground_assets)

    # --- Metadata
    logging.info("Collecting and storing metadata for each object.")
    kb.write_json(filename=output_dir_i / "metadata.json", data={
        "flags": vars(FLAGS),
...
def main():
    ...
    out_dict = simulate(FLAGS)

    
    traj_positions, traj_lookats = out_dict["camera_traj"]

    for cam_id, (pos, look) in enumerate(zip(traj_positions[1:-1], traj_lookats[1:-1])):
        print(f"[INFO] Rendering camera {cam_id} ...")
        render(cam_id, (pos, look), **out_dict)

    kb.done()

    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"[INFO] Rendering completed in {elapsed_time:.2f} seconds.")
